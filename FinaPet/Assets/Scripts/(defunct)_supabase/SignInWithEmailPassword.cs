	...
	public class SignInWithEmailPassword : MonoBehaviour
	{
		...
		private async Task PerformSignIn()
		{
			...
			catch (GotrueException goTrueException)
			{
                ...
                catch (GotrueException goTrueExceptionIn)
				{
                    ErrorText.text = $"{goTrueExceptionIn.Reason} {goTrueException.Message}";
                    Debug.Log(goTrueExceptionIn.Message, gameObject);
                    Debug.LogException(goTrueExceptionIn, gameObject);
                }
            }
			catch (Exception e)
			{
